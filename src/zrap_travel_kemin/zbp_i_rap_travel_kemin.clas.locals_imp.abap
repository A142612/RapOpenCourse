...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD get_instance_features.
   " Read the travel status of the existing travels
    READ ENTITIES OF zi_rap_travel_kemin IN LOCAL MODE
      ENTITY Travel
  ...
  METHOD acceptTravel.
   " Set the new overall status
    MODIFY ENTITIES OF zi_rap_travel_kemin IN LOCAL MODE
      ENTITY Travel
         UPDATE
           FIELDS ( TravelStatus )
   ...
   " Read relevant travel instance data
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-AgencyID IS INITIAL OR NOT line_exists( agencies_db[ agency_id = travel-AgencyID ] ).
        ...
        APPEND VALUE #( %tky        = travel-%tky
                        %state_area = 'VALIDATE_AGENCY'
                        %msg        = NEW zcm_rap_kemin(
                                          severity = if_abap_behv_message=>severity-error
                                          textid   = zcm_rap_kemin=>agency_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-CustomerID IS INITIAL OR NOT line_exists( customers_db[ customer_id = travel-CustomerID ] ).
        ...
        APPEND VALUE #(  %tky        = travel-%tky
                         ...
                         %msg        = NEW zcm_rap_kemin(
                                           severity   = if_abap_behv_message=>severity-error
                                           textid     = zcm_rap_kemin=>customer_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-EndDate < travel-BeginDate.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_kemin(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_kemin=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_kemin(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_kemin=>begin_date_before_system_date
                                                 begindate = travel-BeginDate )
...
*   For the Travel Status we need the before-image from the database. We perform this for active (is_draft=00) as well as for drafts (is_draft=01) as we can't distinguish between edit or new drafts
   ...
   IF is_update_requested = abap_true.
         ...
         IF has_before_image = abap_true.
           ...
           IF update_granted = abap_false.
             APPEND VALUE #( %tky        = travel-%tky
                             %msg        = NEW zcm_rap_kemin( severity = if_abap_behv_message=>severity-error
                                                             textid   = zcm_rap_kemin=>unauthorized )
                           ) TO reported-travel.
           ENDIF.
  ...
  METHOD is_create_granted.
   ...
   AUTHORITY-CHECK OBJECT 'ZOSTAKEMI'
      ID 'ZOSTATKEMI' DUMMY
      ID 'ACTVT' FIELD '01'.
  ...
  METHOD is_delete_granted.
   ...
   IF has_before_image = abap_true.
      AUTHORITY-CHECK OBJECT 'ZOSTATKEMI'
        ID 'ZOSTATKEMI' FIELD overall_status
        ID 'ACTVT' FIELD '06'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZOSTATKEMI'
        ID 'ZOSTATKEMI' DUMMY
