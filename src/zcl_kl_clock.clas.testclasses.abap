...
CLASS ltcl_clock DEFINITION FOR TESTING RISK LEVEL HARMLESS DURATION SHORT FINAL.
  PRIVATE SECTION.
    DATA cut TYPE REF TO zcl_kl_clock.
    METHODS:
...
CLASS ltcl_clock IMPLEMENTATION.
  METHOD on_the_hour.
    cut = NEW zcl_kl_clock( 8 ).
    cl_abap_unit_assert=>assert_equals(
        exp = '08:00'
        act = cut->get( ) ).
  ENDMETHOD.
  METHOD past_the_hour.
    cut = NEW zcl_kl_clock( hours   = 11
                         minutes = 9 ).
    cl_abap_unit_assert=>assert_equals(
        exp = '11:09'
        act = cut->get( ) ).
  ...
  METHOD equivalent_clocks.
    cl_abap_unit_assert=>assert_equals(
        exp = NEW zcl_kl_clock( hours   = 15
                             minutes = 37 )->get( )
        act = NEW zcl_kl_clock( hours   = 15
                             minutes = 37 )->get( ) ).
  ENDMETHOD.
  METHOD inequivalent_clocks.
    cl_abap_unit_assert=>assert_differs(
        exp = NEW zcl_kl_clock( hours   = 1
                             minutes = 1 )->get( )
        act = NEW zcl_kl_clock( hours   = 18
                             minutes = 32 )->get( ) ).
  ENDMETHOD.
  METHOD equivalent_clocks_1.
    cl_abap_unit_assert=>assert_equals(
        exp = NEW zcl_kl_clock( hours   = 3
                             minutes = 11 )->get( )
        act = NEW zcl_kl_clock( hours   = 99
                             minutes = 11 )->get( ) ).
  ENDMETHOD.
  METHOD equivalent_clocks_2.
    cl_abap_unit_assert=>assert_equals(
        exp = NEW zcl_kl_clock( hours   = 22
                             minutes = 40 )->get( )
        act = NEW zcl_kl_clock( hours   = -2
                             minutes = 40 )->get( ) ).
  ENDMETHOD.
  METHOD equivalent_clocks_3.
    cl_abap_unit_assert=>assert_equals(
        exp = NEW zcl_kl_clock( hours   = 17
                             minutes = 3 )->get( )
        act = NEW zcl_kl_clock( hours   = -31
                             minutes = 3 )->get( ) ).
  ENDMETHOD.
  METHOD equivalent_clocks_4.
    cl_abap_unit_assert=>assert_equals(
        exp = NEW zcl_kl_clock( hours   = 2
                             minutes = 2 )->get( )
        act = NEW zcl_kl_clock( hours   = 2
                             minutes = 4322 )->get( ) ).
  ENDMETHOD.
  METHOD equivalent_clocks_5.
    cl_abap_unit_assert=>assert_equals(
        exp = NEW zcl_kl_clock( hours   = 2
                             minutes = 40 )->get( )
        act = NEW zcl_kl_clock( hours   = 3
                             minutes = -20 )->get( ) ).
  ENDMETHOD.
  METHOD equivalent_clocks_6.
    cl_abap_unit_assert=>assert_equals(
        exp = NEW zcl_kl_clock( hours   = 7
                             minutes = 32 )->get( )
        act = NEW zcl_kl_clock( hours   = -12
                             minutes = -268 )->get( ) ).
  ENDMETHOD.
  METHOD hours_rollover.
    cl_abap_unit_assert=>assert_equals(
        exp = '04:00'
        act = NEW zcl_kl_clock( 100 )->get( ) ).
  ENDMETHOD.
  METHOD minutes_rollover.
    cl_abap_unit_assert=>assert_equals(
        exp = '04:43'
        act = NEW zcl_kl_clock( hours   = 0
                             minutes = 1723 )->get( ) ).
  ENDMETHOD.
  METHOD hours_and_minutes_rollover.
    cl_abap_unit_assert=>assert_equals(
        exp = '00:00'
        act = NEW zcl_kl_clock( hours   = 72
                             minutes = 8640 )->get( ) ).
  ENDMETHOD.
  METHOD negative_hours_rollover.
    cl_abap_unit_assert=>assert_equals(
        exp = '05:00'
        act = NEW zcl_kl_clock( -91 )->get( ) ).
  ENDMETHOD.
  METHOD negative_minutes_rollover.
    cl_abap_unit_assert=>assert_equals(
        exp = '16:40'
        act = NEW zcl_kl_clock( hours   = 1
                             minutes = -4820 )->get( ) ).
  ENDMETHOD.
  METHOD negative_hours_and_minutes.
    cl_abap_unit_assert=>assert_equals(
        exp = '22:10'
        act = NEW zcl_kl_clock( hours   = -121
                             minutes = -5810 )->get( ) ).
  ENDMETHOD.
ENDCLASS.
